...
1. [SQLite Internals: Pages & B-trees](https://fly.io/blog/sqlite-internals-btree/) by fly.io
1. Julia Evans's [How does SQLite work?](https://jvns.ca/blog/2014/09/27/how-does-sqlite-work-part-1-pages) takes a very
   different approach to the exploratory problem and the blog is nicely complimentary.
2. [Build your own SQLite](https://blog.sylver.dev/build-your-own-sqlite-part-1-listing-tables) by
   Geoffrey Copin covers a whole lot more than I did, but our implementations differs a lot.
3. [Let's Build a Simple Database](https://cstack.github.io/db_tutorial/) has some useful info, but the project is now
   unmaintained. The spiritual successor is [CodeCrafter's Build your own
   SQLite](https://app.codecrafters.io/courses/sqlite/stages/sz4) project, but I
   personally won't recommend it. The progression b/w subsequent steps are
   non linear and it's not easy to follow through.
4. [SQLite File Format](https://torymur.github.io/sqlite-repr) might help if you
   are starting from scratch again.
5. Definitely SQLite source. See the [DB Header
   description](https://github.com/sqlite/sqlite/blob/e69b4d7/src/btreeInt.h#L45-L82)
